...
/// <summary>
/// Represents a Redis-based implementation of <see cref="IStateStore"/>.
/// </summary>
/// <param name="options">The service used to access the options used to configure the <see cref="RedisStateStore"/>.</param>
/// <param name="connectionMultiplexer">The service used to interact with the Redis server to use.</param>
public sealed class RedisStateStore(IOptions<RedisStateStoreOptions> options, IConnectionMultiplexer connectionMultiplexer)
    : IStateStore
{
    ...
    public async Task<Models.TaskQueryResult> ListTaskAsync(Models.TaskQueryOptions? queryOptions = null, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < values.Length; i++)
        {
            ...
            task = ApplyTaskProjection(task, queryOptions);
            tasks.Add(task);
        }
    ...
    static Models.Task ApplyTaskProjection(Models.Task task, Models.TaskQueryOptions queryOptions)
    {
