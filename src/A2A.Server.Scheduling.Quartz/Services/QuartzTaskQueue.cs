    ...
    : IA2ATaskQueue
{
    ...
    /// <inheritdoc/>
    public async Task EnqueueAsync(Models.Task task, string? tenant = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(task);
        var job = JobBuilder.Create<QuartzTaskExecutionJob>()
            .WithIdentity(new JobKey(GetJobKey(task.Id, tenant)))
            .UsingJobData(QuartzTaskExecutionJob.TaskId, task.Id)
            .UsingJobData(QuartzTaskExecutionJob.Tenant, tenant)
            .Build();
    ...
    public async Task CancelAsync(Models.Task task, string? tenant = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(task);
        var scheduler = await schedulerFactory.GetScheduler(cancellationToken).ConfigureAwait(false);
        await scheduler.DeleteJob(new JobKey(GetJobKey(task.Id, tenant)), cancellationToken).ConfigureAwait(false);
    }
