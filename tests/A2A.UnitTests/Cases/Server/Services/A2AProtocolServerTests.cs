...
public class A2AProtocolServerTests
{
    ...
    {
        //arrange
        var server = A2AProtocolServerFactory.Create();
    ...
    {
        //arrange
        var pushNotificationSender = new Mock<IPushNotificationSender>(MockBehavior.Strict);
        ...
        var server = A2AProtocolServerFactory.Create(pushNotificationSender: pushNotificationSender.Object);
    ...
    public async Task SendMessage_For_NonExistingTask_Should_Throw()
    {
        ...
        var server = A2AProtocolServerFactory.Create(store: store.Object);
        var request = SendMessageRequestFactory.Create();
